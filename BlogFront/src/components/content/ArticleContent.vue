<template>
  <div class="article-content layout-content" v-if="article">
    <Row>
      <Column :xs="24" :sm="24" :md="24" :lg="ExpandLeftColumn ? 24 : 17">
...
<!--                  {{article}}-->
      ...
      <Column :xs="0" :sm="0" :md="0" :lg="ExpandLeftColumn ? 0 : 7">
  ...
  methods: {
    ...
    refreshData() {
      ...
      this.getArticleDetailInfo(this.id).then(response => {
        this.refreshContent();
        // window.console.log("refreshData response",response)
      });
    ...
    // 更新目录
    refreshContent() {
      document.title = this.article.title + " | ClockBlog";
    ...
    printArticle(){
      window.console.log("state.article.article=",state.article.article)
