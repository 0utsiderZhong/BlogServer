<template>
  <div class="custom-mavon-editor">
    <div class="operate">
      <Form>
        <Row>
          <Column :xs="10" :sm="10" :md="10" :lg="10" style="padding-left: 0; padding-right: 7.5px;">
            <InputComponent placeholder="please enter your QQ number" v-model="form.qqNum" clearable @on-blur="getInfoByQQ"
                   size="default">
              <span slot="prepend">Q  Q</span>
            </InputComponent>
          </Column>
          <Column :xs="4" :sm="4" :md="4" :lg="4" style="padding-left: 0; padding-right: 0;">
            <Avatar icon="ios-person" :src="form.avatar==undefined?'':form.avatar"/>
          </Column>
          <Column :xs="10" :sm="10" :md="10" :lg="10">
            <InputComponent v-model="form.nickName" placeholder="* please enter your name" size="default">
        ...
        <Row>
          <Column :xs="10" :sm="10" :md="10" :lg="10">
            <InputComponent v-model="form.email" placeholder="* please enter your email" size="default">
              <span slot="prepend">Email</span>
            </InputComponent>
          </Column>
          <Column :xs="6" :sm="6" :md="6" :lg="6">
            <Checkbox v-model="form.reply" style=" margin: 5% 8%;">Email notification</Checkbox>
          </Column>
        </Row>
      </Form>
    </div>
    ...
    <div class="editor-area">
      <Spin size="large" v-if="!allowComment" fix style="z-index: 1001;">
        The comment function is closed in this Blog
      </Spin>
    ...
    <div class="bottom-area">
      ...
      <div class="publish-area">
        <Button size="default" :type="buttonType" :loading="publishing" @click="send"
          ...
          <span v-if="!publishing">Done</span>
          <span v-else>Doing</span>
        </Button>
      </div>
    </div>
  </div>
        ...
        getInfoByQQ(this.form.qqNum).then(response => {
          if (response.code === 200) {
            if (response.data.nickName === '') {
              this.$Message.error("Get msg failed, the account has not been used in the system");
              this.form.avatar = '';
            } else {
              this.$Message.info("Get msg successfully");
